...
namespace NetLib.RTP
{
    ...
    public class RTP_ReceiveStream
    {
        ...
        internal void Process(RTP_Packet packet, int size)
        {
            ...
            if (UpdateSeq(packet.SeqNo))
            {
                ...
                */
                ...
                var transit = (int) (arrival - packet.Timestamp);
        ...
        private bool UpdateSeq(ushort seqNo)
        {
            ...
            var udelta = (ushort) (seqNo - m_MaxSeqNo);
        ...
        internal RTCP_Packet_ReportBlock CreateReceiverReport()
        {
            ...
            */
            ...
            var extHighestSeqNo = (uint) (m_SeqNoWrapCount << 16 + m_MaxSeqNo);
            uint expected = extHighestSeqNo - m_BaseSeq + 1;

            var expected_interval = (int) (expected - m_ExpectedPrior);
            m_ExpectedPrior = expected;
            var received_interval = (int) (m_PacketsReceived - m_ReceivedPrior);
        ...
        public long PacketsLost
        {
            get
            {
                ...
                // RFC 3550 A.3 Determining Number of Packets Expected and Lost.                
                var extHighestSeqNo = (uint) ((65536*m_SeqNoWrapCount) + m_MaxSeqNo);
                uint expected = extHighestSeqNo - m_BaseSeq + 1;
        ...
        public int DelaySinceLastSR
        {
            get
            {
                ...
                return (int) (m_LastSRTime == DateTime.MinValue ? -1 : ((DateTime.Now - m_LastSRTime)).TotalMilliseconds);
            }
        }
