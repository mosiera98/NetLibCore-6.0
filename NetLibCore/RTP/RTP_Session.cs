...
using NetLib.Media.Codec;
using NetLib.STUN.Client;
using NetLib.UDP;
...
namespace NetLib.RTP
{
    ...
    public class RTP_Session : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            foreach (var receiver in m_pUdpDataReceivers)
                receiver.Dispose();
        ...
        public void Close(string closeReason)
        {
            ...
            var rr = new RTCP_Packet_RR {SSRC = m_pRtcpSource.SSRC};
            ...
            while (sourcesProcessed < m_pLocalSources.Count)
            {
                ...
                var bye = new RTCP_Packet_BYE {Sources = sources};
        ...
        public void Start()
        {
            ...
            */
            ...
            rtpDataReceiver.PacketReceived += (s1, e1) => ProcessRtp(e1.Buffer, e1.Count, e1.RemoteEP);
            ...
            var rtcpDataReceiver = new UDP_DataReceiver(m_pRtcpSocket, true);
            rtcpDataReceiver.PacketReceived += (s1, e1) => ProcessRtcp(e1.Buffer, e1.Count, e1.RemoteEP);
        ...
        private RTP_Source_Local CreateLocalSource()
        {
            ...
            */
            ...
            lock (m_pMembers)
            {
                ...
                else if (!packetEP.Equals((rtcp_rtp ? source.RtcpEP : source.RtpEP)))
                {
                    ...
                    var bye = new RTCP_Packet_BYE {Sources = new[] {oldSSRC}, LeavingReason = "Collision, changing SSRC."};
        ...
        private void DoReverseReconsideration()
        {
            ...
            */
            ...
            Schedule((int) Math.Max((m_pMembers.Count/m_PMembersCount)*((timeNext - DateTime.Now)).TotalSeconds, 2));
        ...
        private void SendRtcp()
        {
            ...
            */
            ...
            try
            {
                ...
                {
                    rr = new RTCP_Packet_RR {SSRC = m_pRtcpSource.SSRC};
                    compundPacket.Packets.Add(rr);
        ...
        /// <exception cref="ObjectDisposedException">Is raised when this class is Disposed and this property is accessed.</exception>
        public IEnumerable<RTP_Address> Targets
        {
            get
            {
                if (m_IsDisposed)
        ...
        /// </summary>
        /// <exception cref="ObjectDisposedException">Is raised when this class is Disposed and this property is accessed.</exception>
        public IEnumerable<RTP_SendStream> SendStreams
        {
            get
            {
                if (m_IsDisposed)
