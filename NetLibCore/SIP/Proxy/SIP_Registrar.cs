            ...
            */
                ...
                if (request.Contact.GetAllValues().Length > 1)
                {
                    transaction.SendResponse(
                        m_pStack.CreateResponse(
                            ...
                            ": RFC 3261 10.3.6 -> If star(*) present, only 1 contact allowed.", request));
                    return;
                }
            ...
            if (starContact == null)
            {
                ...
                foreach (SIP_t_ContactParam c in request.Contact.GetAllValues())
                {
                    ...
                    {
                        transaction.SendResponse(
                            m_pStack.CreateResponse(SIP_ResponseCodes.x400_Bad_Request + ": CSeq value out of order.",
                                                    request));
                        return;
                    }
                }
                ...
                // Do binding updates.
                reg.AddOrUpdateBindings(e.ServerTransaction.Flow, request.CallID, request.CSeq.SequenceNumber,
