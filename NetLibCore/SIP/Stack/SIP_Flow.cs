    ...
    public class SIP_Flow : IDisposable
    {
        ...
        public void SendPing()
        {
            lock (m_pLock)
            {
                ...
                {
                    m_pStack.TransportLayer.Stack.Logger.AddWrite("", null, 2,
                                                 ...
                                                 lock (m_pLock)
                                                 {
                                                         ...
                                                         m_Transport != SIP_Transport.UDP)
                                                     {
                                                         try
                                                         {
                                                             ...
                                                             client.Connect(m_pLocalEP, m_pRemoteEP,
                                                                            m_Transport ==
                                                                            SIP_Transport.TLS);
        ...
        private void BeginReadHeader_Completed(IAsyncResult asyncResult)
        {
            try
            {
                ...
                else
                {
                    ...
                    string contentLengthValue =  MIME.MIME_Utils.ParseHeaderField("Content-Length:",  m_pMessage);
                    ...
                    {
                        // Read body data.
                        m_pTcpSession.TcpStream.BeginReadFixedCount(m_pMessage, contentLength,
            ...
            get
            {
                ...
                try
                {
                    ...
                    SIP_Request optionsRequest = m_pStack.CreateRequest(SIP_Methods.OPTIONS,
                                                                        new SIP_t_NameAddress(
                                                                            "sip:ping@publicIP.com"),
                                                                        new SIP_t_NameAddress(
                                                                            "sip:ping@publicIP.com"));
                    optionsRequest.MaxForwards = 0;
                    SIP_ClientTransaction optionsTransaction =
                        m_pStack.TransactionLayer.CreateClientTransaction(this, optionsRequest, true);
                    optionsTransaction.ResponseReceived +=
                        delegate(object s, SIP_ResponseReceivedEventArgs e)
                            {
                                SIP_t_ViaParm via = e.Response.Via.GetTopMostValue();

                                var publicEP =
                                    new IPEndPoint(via.Received ?? LocalEP.Address,
                                                   via.RPort > 0 ? via.RPort : LocalEP.Port);
