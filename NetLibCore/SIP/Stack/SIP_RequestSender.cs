        ...
        private void ClientTransaction_ResponseReceived(object sender, SIP_ResponseReceivedEventArgs e)
        {
            lock (m_pLock)
            {
                ...
                if (e.Response.StatusCode == 401 || e.Response.StatusCode == 407)
                {
                    ...
                    {
                        foreach (
                            SIP_t_Credentials credentials in m_pTransaction.Request.ProxyAuthorization.GetAllValues())
                        {
                            if (new Auth_HttpDigest(challange.AuthData, "").Realm ==
                                new Auth_HttpDigest(credentials.AuthData, "").Realm)
                            {
        ...
        private void ClientTransaction_TransportError(object sender, EventArgs e)
        {
            lock (m_pLock)
            {
                ...
                */
                ...
                {
                    OnResponseReceived(
                        m_pStack.CreateResponse(SIP_ResponseCodes.x503_Service_Unavailable + ": Transport error.",
                                                m_pRequest));
                    OnCompleted();
                }
            }
            ...
            lock (m_pLock)
            {
                ...
                ThreadPool.QueueUserWorkItem(delegate
                        ...
                        */
                                                         ...
                                                         else if (m_pRequest.Route.GetTopMostValue() !=
                                                                  null)
                                                         {
                                                             uri =
                                                                 (SIP_Uri)
                                                                 m_pRequest.Route.GetTopMostValue()
                                                                     .Address.Uri;
                                                         }
                                                         else
                                                         {
                                                         ...
                                                         {
                                                             OnTransportError(
                                                                 new SIP_TransportException(
                                                                     "SIP hops resolving failed '" +
                                                                     x.Message + "'."));
                                                             OnCompleted();
        ...
        private void SendToFlow(SIP_Flow flow, SIP_Request request)
        {
            ...
            */
            ...
            if (SIP_Utils.MethodCanEstablishDialog(request.RequestLine.Method) && contact == null)
            {
                ...
                request.Contact.Add((flow.IsSecure ? "sips:" : "sip:") + from.User + "@" + flow.LocalPublicEP);
