...
using NetLib.DNS;
using NetLib.DNS.Client;
using NetLib.SIP.Message;
using NetLib.TCP;
using NetLib.UDP;
using NetLib.URI;
using NetLib._Obsolete.ServersCore;
    ...
    public class SIP_TransportLayer
    {
        ...
        private class SIP_FlowManager : IDisposable
        {
            ...
            internal SIP_FlowManager(SIP_TransportLayer owner)
            {
                ...
                m_pTimeoutTimer = new TimerEx(15000) {AutoReset = true};
            ...
            internal SIP_Flow CreateFromSession(TCP_ServerSession session)
            {
                ...
                string flowID = session.LocalEndPoint.ToString() + "-" + session.RemoteEndPoint.ToString() + "-" +
        ...
        private void m_pUdpServer_PacketReceived(object sender, UDP_e_PacketReceived e)
        {
            try
            {
                SIP_Flow flow = m_pFlowManager.GetOrCreateFlow(true, (IPEndPoint) e.Socket.LocalEndPoint, e.RemoteEP,
                                                               SIP_Transport.UDP);
        ...
        internal void OnMessageReceived(SIP_Flow flow, byte[] message)
        {
            ...
            try
            {
                ...
                if (message.Length == 4)
                {
                    ...
                    {
                        Stack.Logger.AddRead("", null, 2, "Flow [id='" + flow.ID + "'] received \"ping\"",
                                                  flow.LocalEP, flow.RemoteEP);
                    }
                    ...
                    if (Stack.Logger != null)
                    {
                        Stack.Logger.AddWrite("", null, 2, "Flow [id='" + flow.ID + "'] sent \"pong\"",
                                                   flow.LocalEP, flow.RemoteEP);
                    }
                ...
                if (message.Length == 2)
                {
                    if (Stack.Logger != null)
                    {
                        Stack.Logger.AddRead("", null, 2, "Flow [id='" + flow.ID + "'] received \"pong\"",
                                                  flow.LocalEP, flow.RemoteEP);
                    }
                ...
                else
                {
                    ...
                    catch (Exception x)
                    {
                        ...
                        SendResponse(m_pStack.CreateResponse(SIP_ResponseCodes.x400_Bad_Request + ". " + x.Message,
                                                             request));
                    ...
                    if (!processed)
                    {
                        ...
                        if (m_pStack.Logger != null)
                        {
                            ...
                            m_pStack.Logger.AddRead(
                                ...
                                null,
                                0,
                                "Request [method='" + request.RequestLine.Method + "'; cseq='" +
        ...
        internal void SendRequest(SIP_Flow flow, SIP_Request request, SIP_ClientTransaction transaction)
        {
            ...
            foreach (IPBindInfo bind in BindInfo)
            {
                ...
                {
                    sentBy = !string.IsNullOrEmpty(bind.HostName)
                                 ? new HostEndPoint(bind.HostName, bind.Port)
                                 : new HostEndPoint(flow.LocalEP.Address.ToString(), bind.Port);
                    break;
                }
                if (flow.Transport == SIP_Transport.TLS && bind.Protocol == BindInfoProtocol.TCP &&
            ...
            via.SentBy = sentBy ?? new HostEndPoint(flow.LocalEP);
            ...
            if (m_pStack.Logger != null)
            {
                ...
                m_pStack.Logger.AddWrite(
                    ...
                    null,
                    0,
                    "Request [" + (transaction == null ? "" : "transactionID='" + transaction.ID + "';") + "method='" +
                    request.RequestLine.Method + "'; cseq='" + request.CSeq.SequenceNumber + "'; " +
                    "transport='" + flow.Transport + "'; size='" + requestData.Length + "'; sent '" + flow.LocalEP +
                    "' -> '" + flow.RemoteEP + "'.",
                    flow.LocalEP,
                    flow.RemoteEP,
                    requestData
        ...
        private void SendResponseInternal(SIP_ServerTransaction transaction, SIP_Response response, IPEndPoint localEP)
        {
            ...
            */
                ...
                try
                {
                    ...
                    if (m_pStack.Logger != null)
                    {
                        m_pStack.Logger.AddWrite(
                            ...
                            null,
                            0,
                            "Response [flowReuse=true; transactionID='" + transactionID + "'; method='" +
                            response.CSeq.RequestMethod + "'; cseq='" + response.CSeq.SequenceNumber + "'; " +
                            "transport='" + flow.Transport + "'; size='" + responseData.Length + "'; statusCode='" +
                            response.StatusCode + "'; " +
            ...
            if (SIP_Utils.IsReliableTransport(via.ProtocolTransport))
            {
                    ...
                    if (flow != null)
                    {
                        ...
                        if (m_pStack.Logger != null)
                        {
                            m_pStack.Logger.AddWrite(
                                ...
                                "transport='" + flow.Transport + "'; size='" + responseData.Length + "'; statusCode='" +
                                response.StatusCode + "'; " +
                                "reason='" + response.ReasonPhrase + "'; sent '" + flow.RemoteEP + "' -> '" +
                                flow.LocalEP + "'.",
                                localEP,
                                remoteEP,
                                responseData
                ...
                if (remoteEP != null)
                {
                    try
                    {
                        SendResponseToHost(logID, transactionID, null, remoteEP.Address.ToString(), remoteEP.Port,
                                           via.ProtocolTransport, response);
                    }
                    catch
                    {
           ...
           /* else if (via.Maddr != null)
            ...
            {
                SendResponseToHost(logID, transactionID, localEP, via.Received.ToString(), via.RPort,
                                   via.ProtocolTransport, response);
            }
            ...
            {
                SendResponseToHost(logID, transactionID, localEP, via.Received.ToString(), via.SentByPortWithDefault,
                                   via.ProtocolTransport, response);
            }
        ...
        /// <param name="response">SIP response.</param>
        /// <exception cref="SIP_TransportException">Is raised when <b>response</b> sending has failed.</exception>
        private void SendResponse_RFC_3263_5(string logID, string transactionID, IPEndPoint localEP,
                                             SIP_Response response)
        {
            /* RFC 3263 5.
            ...
            {
                SendResponseToHost(logID, transactionID, localEP, via.SentBy.Host, via.SentByPortWithDefault,
                                   via.ProtocolTransport, response);
            }
                            ...
                            try
                            {
                                if (m_pStack.Logger != null)
                                {
                                    m_pStack.Logger.AddText(logID,
                                                            "Starts sending response to DNS SRV record '" + srv.Target +
                                                            "'.");
                                }

                                SendResponseToHost(logID, transactionID, localEP, srv.Target, srv.Port,
                                                   via.ProtocolTransport, response);
                            }
                            catch
                            {
        ...
        /// <param name="response">SIP response to send.</param>
        private void SendResponseToHost(string logID, string transactionID, IPEndPoint localEP, string host, int port,
                                        string transport, SIP_Response response)
        {
            try
            {
                ...
                for (int i = 0; i < targets.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (m_pStack.Logger != null)
                        {
                            m_pStack.Logger.AddWrite(
                                ...
                                null,
                                0,
                                "Response [transactionID='" + transactionID + "'; method='" +
                                response.CSeq.RequestMethod + "'; cseq='" + response.CSeq.SequenceNumber + "'; " +
                                "transport='" + transport + "'; size='" + responseData.Length + "'; statusCode='" +
                                response.StatusCode + "'; " +
                    ...
                    catch
                    {
                        ...
                        if (i == (targets.Length - 1))
                        {
                            ...
                            {
                                m_pStack.Logger.AddText(logID,
                                                        "Failed to send response to host '" + host + "' IP end point '" +
                                                        remoteEP + "'.");
                            }
                        ...
                        if (m_pStack.Logger != null)
                        {
                            m_pStack.Logger.AddText(logID,
                                                    "Failed to send response to host '" + host + "' IP end point '" +
                                                    remoteEP + "', will try next A record.");
                        }
                    }
                }
            }
        ...
        internal HostEndPoint GetContactHost(SIP_Flow flow)
        {
            ...
            foreach (IPBindInfo bind in BindInfo)
            {
                if (bind.Protocol == BindInfoProtocol.UDP && flow.Transport == SIP_Transport.UDP)
                {
                    ...
                    if (bind.IP.AddressFamily == flow.LocalEP.AddressFamily && bind.Port == flow.LocalEP.Port)
                    {
                        retVal =
                            new HostEndPoint(
                                (string.IsNullOrEmpty(bind.HostName) ? flow.LocalEP.Address.ToString() : bind.HostName),
                                bind.Port);
                        break;
                    }
                }
                            ...
                            retVal =
                                new HostEndPoint(
                                    (string.IsNullOrEmpty(bind.HostName)
                                         ...
                                         : bind.HostName), bind.Port);
                        }
                        else
                        {
                            retVal =
                                new HostEndPoint(
                                    (string.IsNullOrEmpty(bind.HostName) ? bind.IP.ToString() : bind.HostName),
                                    bind.Port);
                        }
                        break;
                    }
            ...
            if (retVal.IsIPAddress && Net_Utils.IsPrivateIP(IPAddress.Parse(retVal.Host)) &&
                !Net_Utils.IsPrivateIP(flow.RemoteEP.Address))
            {
        ...
        /// </summary>
        public IEnumerable<SIP_Flow> Flows
        {
