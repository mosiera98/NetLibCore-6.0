...
using ErrorEventHandler = NetLib._Obsolete.ServersCore.ErrorEventHandler;
...
namespace NetLib.UDP
{
    ...
    public class UDP_Server : IDisposable
    {
        ...
        private int m_ReceiversPerSocket = 100;
            ...
            if (m_pBindings != null)
            {
                ...
                foreach (IPEndPoint ep in listeningEPs)
                {
                    try
                    {
                        ...
                        {
                            var receiver = new UDP_DataReceiver(socket, true);
                            ...
                            receiver.Error += (s, e) => OnError(e.Exception);
